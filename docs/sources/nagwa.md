...
`package.json` declares the ISC license, but the reviewed repository tree did
not contain a standalone license file or license text. Treat reuse as requiring
clarification.

## Public shape

The browser-oriented TypeScript API measures text with Canvas and can transform
an array of strings to approximate the widest member. The API takes a CSS font
descriptor rather than font bytes.
...
6. inserts repeated U+0640 characters before the second character of each
   regex match.
...
- font-family strings are not reproducible across browser and Node runtimes;
